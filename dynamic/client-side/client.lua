...
AddEventHandler("dynamic:animalSpawn",function(model,isPolice)
	if animalHash == nil then
			...
			if myObject then
				...
				if isPolice then
					Citizen.CreateThread(function()
						while vSERVER.isAnimalSpawned() do
							local timeDistance = 500
							local dogCoords = GetEntityCoords(animalHash)
							local distance = #(GetEntityCoords(PlayerPedId()) - dogCoords)
							local ped = PlayerPedId()

							if distance <= 20 then
								timeDistance = 1
								aim,target = GetEntityPlayerIsFreeAimingAt(PlayerId())
								if aim then
									dogTarget = target
									TaskCombatPed(animalHash,dogTarget)
								end

								print(dogTarget)
								if IsPedFalling(dogTarget) then
									ClearPedTasks(animalHash)
									ClearPedSecondaryTask(animalHash)
									TriggerEvent("Notify","amarelo","Cidadão coletado!",8000)
								end
							end
